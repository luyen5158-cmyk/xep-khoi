...
| 排行榜資料 / Dữ liệu | Supabase，只開一個專案 | 🟡 SQL 寫好，等開專案 |
| 榜單種類 / Loại bảng | 只有總榜 | 🟡 程式已完成 |
| 怎麼存 / Cách lưu | 每人只存一筆最高分 | 🟡 程式已完成 |
| 名次 / Thứ hạng | 每人只佔一個名次 | 🟡 程式已完成 |
| 登入 / Đăng nhập | Google 一鍵，用 Supabase 內建 | 🟡 程式已完成 |
| 榜上名字 / Tên hiện trên bảng | 玩家自己取的暱稱 | 🟡 程式已完成 |
| 沒登入 / Không đăng nhập | 可以玩，不能上榜 | ✅ 已完成 |
| 防作弊 / Chống gian lận | 伺服器重跑整局自己算分 | 🟡 兩端程式都完成 |
| 亂數種子 / Hạt giống | **伺服器發，用過作廢** | 🟡 程式已完成 |
| 對局記錄 / Bản ghi ván đấu | **不公開，只有伺服器看得到** | 🟡 程式已完成 |
| 排名局 / Ván xếp hạng | 有登入+有網路自動算，否則變練習局 | ✅ 已完成 |
| 防機器人 / Chống máy tự chơi | 只做基本的（次數限制+伺服器種子） | 🟡 程式已完成 |
| 排行榜畫面 / Màn hình bảng | 前 50 名 + 自己的名次 | ✅ 已完成 |
| 防猜密碼 / Chống dò mật khẩu | 有次數限制 | 🟡 程式已完成 |
