...
**狀態符號：** ✅ 做完了 · 🟡 程式全部寫好、等 Supabase 專案開通（步驟見 [supabase/README.md](supabase/README.md)） · ⬜ 還沒開始。
...
| 排行榜資料 / Dữ liệu | Supabase，只開一個專案 | ✅ 專案已開、migration 已套用 |
| 榜單種類 / Loại bảng | 只有總榜 | ✅ 已上線 |
| 怎麼存 / Cách lưu | 每人只存一筆最高分 | ✅ 已上線 |
| 名次 / Thứ hạng | 每人只佔一個名次 | ✅ 已上線 |
| 登入 / Đăng nhập | Google 一鍵，用 Supabase 內建 | ✅ 已上線並實測通過 |
| 榜上名字 / Tên hiện trên bảng | 玩家自己取的暱稱 | ✅ 已上線 |
| 沒登入 / Không đăng nhập | 可以玩，不能上榜 | ✅ 已完成 |
| 防作弊 / Chống gian lận | 伺服器重跑整局自己算分 | ✅ 已上線並實測通過 |
| 亂數種子 / Hạt giống | **伺服器發，用過作廢** | ✅ 已上線 |
| 對局記錄 / Bản ghi ván đấu | **不公開，只有伺服器看得到** | ✅ 已驗證外部讀不到 |
| 排名局 / Ván xếp hạng | 有登入+有網路自動算，否則變練習局 | ✅ 已完成 |
| 防機器人 / Chống máy tự chơi | 只做基本的（次數限制+伺服器種子） | ✅ 已上線 |
| 排行榜畫面 / Màn hình bảng | 前 50 名 + 自己的名次 | ✅ 已完成 |
| 防猜密碼 / Chống dò mật khẩu | 有次數限制 | ✅ 已上線 |
| 送分時機 / Khi nào gửi điểm | 每局結束都送 | ✅ 已完成 |
| 壞掉時 / Khi hỏng | 顯示「暫時不能用」，遊戲照玩 | ✅ 已完成 |
| 刪除資料 / Xoá dữ liệu | 遊戲裡有刪除按鈕 | ✅ 已完成 |
| 防休眠 / Chống ngủ | GitHub 自動排程定期連線 | 🟡 排程檔已寫好 |
...
| 規則 | 值 | 為什麼 |
|---|---|---|
| 長度 | 2–16 個字元 | 太短認不出人，太長擠爆排行榜那一行 |
| 越南文聲調 | 可以 | 按字元數算，不是位元組數 |
| 唯一性 | **不分大小寫唯一** | 榜上重名是最省事的冒名方式 |
| 隱形字元 | 自動移除 | 控制字元和改變書寫方向的字元會讓整張榜歪掉 |
| 連續空白 | 併成一個 | 避免用空白排版擠掉別人 |
| 髒話過濾 | **不做**（已決定） | 只限長度，後台可手動刪 |

這幾條寫在兩個地方，**改一邊就要改另一邊**：`game/js/leaderboard.js` 的 `NICK_MIN`/`NICK_MAX`/`clean()`，和 `supabase/functions/profile/index.ts` 的同名東西。前端寬鬆後端嚴格會產生最難查的那種 bug —— 畫面數到 6 個字，伺服器卻說太短。

> *Quy tắc biệt danh nằm ở hai nơi, sửa một bên phải sửa bên kia. Lệch nhau là sinh ra loại lỗi khó chịu nhất: màn hình đếm đủ 6 ký tự mà máy chủ vẫn bảo quá ngắn.*
...
- 按設計文件的 8 個 script 分工拆好，PWA 可安裝、可離線
- Service Worker 只處理自己網站的檔案 —— 送去 Supabase 的請求一律不碰
- 防作弊三層全部寫完：伺服器發種子、伺服器重跑算分、對局記錄不公開
- `supabase/schema.sql`：五張表、RLS、四個函式
- 三個 Edge Function：發種子、驗分數、暱稱與刪帳號
- 前端：Google 登入（自己寫的 PKCE，不依賴任何 CDN）、暱稱介面、排行榜畫面、
  排名局／練習局自動判斷、預先拿下一局種子、刪除帳號
- GitHub 排程兩支：防休眠兼清過期種子、檢查兩份規則檔有沒有走鐘

**已驗證**
- 伺服器端引擎在 Node 裡跑同一份 replay，算出的分數與畫面完全一致（2461 = 2461），
  且 `valid` 與 `ended` 都成立 —— 這正是 `submit-score` 收分的條件
- 自動玩完兩局排名局，送分、名次顯示、按「再來一局」瞬間開始排名局，全部正常
- 暱稱含 HTML 標籤時只會顯示成文字，不會變成網頁元素

**還沒做（只剩要帳號才能做的）**
- 開 Supabase 專案、跑 SQL、開 Google 登入、填白名單、部署 Edge Function、
  填 `game/js/config.js` 兩個值、設 GitHub Secrets
  → 步驟全部寫在 [supabase/README.md](supabase/README.md)
- 隱私權政策
- 網域

**還沒驗證**
- 真人用手指在實機上拖放的手感。目前只用程式模擬觸控測過，幾何計算正確，但「順不順手」測不出來。
- PWA 裝成 App 後的 Google 登入。這題只能實機測。
...
- **`drag.js` 那個 `isBusy()` 檢查不准拿掉。** 消行動畫跑的 340 毫秒之內，被消掉的
  格子還佔在盤面上。這時候讓玩家放下一塊，畫面看起來合法，但伺服器重跑時那些格子
  早就清掉了 —— 於是每一局排名局都會被判「無效的下法」而退回，錯誤訊息還完全指不到
  真正的原因。（開發時用機器人自動玩，忽略這個鎖，馬上就重現了。）
- 兩份 `sim.js` 必須一模一樣。改完 `game/js/` 裡任何一個規則檔，要跑
  `./tools/sync-engine.sh` 再 commit，GitHub 上有排程會擋。
